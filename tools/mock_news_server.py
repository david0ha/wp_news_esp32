#!/usr/bin/env python3
"""
The news snapshot contract, as a runnable server.

This is the reference implementation of what the device fetches — the thing a
real WP News plugin or a cron script has to imitate. It exists for three jobs:

  1. Point a board (or the simulator) at a URL and watch real polling work,
     without writing anything that touches a news.
...
-----
    ...
    python3 tools/mock_news_server.py --live          # numbers drift each poll
...
def main():
    ...
    print(f"serving the news snapshot on http://{args.host}:{args.port}/news.json"
