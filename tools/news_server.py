#!/usr/bin/env python3
"""
Serve a REAL WP News news to the board.

`mock_news_server.py` is the contract as a fixed payload — it pins the wire
format and produces the test fixture. This is the other half: it walks an actual
news on disk, works out the same numbers from the notes that are really there,
and serves them at the same URL shape. Point the board at this and the panel
stops showing a demo and starts showing your news.

    python3 tools/news_server.py ~/Documents/MyNews
...
adds one write path, `POST /capture`, which appends a memo to the news's inbox
so the board's queue is somewhere you can actually put things — see "Capture"
below. It stays off unless you ask for it.

What the numbers mean
---------------------
The definitions are WP News's where WP News has one, and stated here where it
does not — the board has no room to explain itself, so this is where the meaning
lives.

- **notes**     every `.md` file under the news, excluding dotted directories
                (`.wp_news`, `.trash`, `.git`) and anything `--exclude`d.
...
- **orphans**   notes with no links in *or* out — WP News's own definition.
...
reopened. A 3,000-note news costs a second the first time and milliseconds
...
That writes `Inbox/ring the dentist.md` into the news, and the board shows it
...
Usage
-----
    python3 tools/news_server.py NEWS                  # serve on :8123
    python3 tools/news_server.py NEWS --port 9000
    python3 tools/news_server.py NEWS --dump           # print the payload
    python3 tools/news_server.py NEWS --inbox Inbox --inbox-tag '#todo'
    python3 tools/news_server.py NEWS --agents ~/agents.json
    python3 tools/news_server.py NEWS --allow-capture  # enable POST /capture
"""
...
# Device-side caps (components/news_core/include/news_model.h). The parser
...
# Directory names that are never news content. Anything else dotted is skipped
# too — `.wp_news` is only the most obvious one.
SKIP_DIRS = {".wp_news", ".trash", ".git", ".github", "node_modules"}
...
# out) and not a heading (`# ` has a space). WP News allows nesting with '/'.
...
def strip_code(text):
    ...
    WP News does not index a `[[link]]` inside a code fence, and neither does
...
def parse_note(path, text):
    """Everything one note contributes, independent of the rest of the news."""
...
class News:
    """A scanned news, with a per-file cache so repeat polls are cheap."""
    ...
    def build_index(notes):
        ...
        WP News resolves `[[Note]]` by basename across the news. When two
    ...
    def snapshot(self, name=None, agents=None, now=None):
        ...
        return {
            "schema": 1,
            "news": name or os.path.basename(self.root),
    ...
    def _graph(self, notes, out_links, degree):
        ...
        degree, with the title as the tie-break so the same news always draws
        ...
        # against the whole news. A graph label is about ten characters wide, so
...
def title_of(rel):
    """The note's display name: its basename, as WP News shows it."""
    return os.path.splitext(os.path.basename(rel))[0][:TITLE_MAX]


def build_titles(notes):
    """Display titles for a whole news, disambiguated where they collide.

    A basename is the right title right up until the news has three notes
    called `README` — and real newss do, along with `index`, `notes`, and a
    ...
    (`docs/README`). WP News's own file switcher does the same thing for the
    same reason.
...
SLUG_MAX = 40
# Characters no filesystem, sync client or WP News link should have to deal
...
def capture(root, folder, text, tag=None, now=None):
    """Write one memo into the news's capture folder. Returns its path.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            path = capture(self.news.root, self.capture_folder, text,
                           tag=self.news.inbox_tag)
        ...
        rel = os.path.relpath(path, self.news.root)
    ...
    def do_GET(self):
        if self.path.split("?")[0] not in ("/news.json", "/"):
            self.send_error(404)
            return
        payload = self.news.snapshot(
            name=self.news_name, agents=load_agents(self.agents_path)
        )
...
def main():
    ...
    ap.add_argument("news", help="path to the WP News news directory")
    ap.add_argument("--port", type=int, default=8123)
    ap.add_argument("--host", default="0.0.0.0")
    ap.add_argument("--name", help="news name shown on the panel (default: folder name)")
    ...
    ap.add_argument("--allow-capture", action="store_true",
                    help="enable POST /capture, which WRITES a memo into the news")
    ap.add_argument("--capture-folder",
                    help="folder captured memos land in (default: the first --inbox, or Inbox)")
    args = ap.parse_args()

    if not os.path.isdir(os.path.expanduser(args.news)):
        sys.exit(f"not a directory: {args.news}")

    news = News(args.news, excludes=args.exclude,
                  inbox_names=args.inbox or None, inbox_tag=args.inbox_tag)
    charset = None if args.no_glyph_check else load_device_charset()

    if args.dump:
        payload = news.snapshot(name=args.name, agents=load_agents(args.agents))
    ...
    print(f"serving it on http://{args.host}:{args.port}/news.json")
