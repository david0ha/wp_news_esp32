#!/usr/bin/env python3
"""
Tests for tools/news_server.py, against a news built in a temp directory.

    python3 tools/test_news_server.py

No framework and no network: it writes a small news whose right answers are
...
payload this produces and a payload `mock_news_server.py` produces are the same
shape.
"""
...
from news_server import (  # noqa: E402
    CAPTURE_MAX_BYTES, CaptureError, News, build_titles, capture, check_glyphs,
...
# ---------------------------------------------------------------------------
# A news whose answers are countable by hand
# ---------------------------------------------------------------------------
...
""",
    ...
    ".wp_news/workspace.md": "not news content [[MOC]]",
...
def test_snapshot(news):
    snap = news.snapshot(now=NOW)
    s = snap["stats"]

    # Eight .md files; the .wp_news one and the .txt are not notes.
...
def test_graph_is_deterministic(news):
    a = json.dumps(news.snapshot(now=NOW)["graph"], ensure_ascii=False, sort_keys=True)
    fresh = News(news.root, inbox_tag="#todo")
    ...
    eq(a, b, "the same news draws the same graph on a cold scan")
...
def test_titles_disambiguate_on_collision():
    ...
    A real news has many notes with the same basename — README, index, a daily
...
def test_graph_titles_disambiguate_only_against_the_drawn_nodes():
    ...
    against the fourteen nodes it actually draws, not against the whole news:
    a note whose basename is unique among those keeps the short, informative
    form even if some note elsewhere in the news shares it.
    """
    ...
    try:
        ...
        snap = News(root).snapshot(now=NOW)
        drawn = {n["title"] for n in snap["graph"]["nodes"]}
        eq(len(snap["graph"]["nodes"]), 14, "the graph is at its cap")
        # Both Note.md files exist, so a news-wide list would disambiguate both
        ...
        # A news-wide list, which has a wider column, still disambiguates.
        news_wide = build_titles(["Sub/Note.md", "Archive/Note.md"])
        eq(news_wide["Sub/Note.md"], "Sub/Note", "the news-wide title is still qualified")
    finally:
        shutil.rmtree(root)


def test_recent(news):
    snap = news.snapshot(now=NOW)
    eq(len(snap["recent"]), 8, "recent is capped at eight and the news has eight")
    for r in snap["recent"]:
        check(len(r["time"]) == 5 and r["time"][2] == ":", "recent time is HH:MM", r["time"])
        check(r["links"] >= 0, "recent link count is a count")


def test_incremental_rescan(news):
    """A second scan must not re-read unchanged files, and must see changed ones."""
    first = news.snapshot(now=NOW)
    cached = dict(news.cache)
    second = news.snapshot(now=NOW)
    eq(second["stats"], first["stats"], "an unchanged news gives an identical answer")
    check(all(news.cache[p][0] == cached[p][0] for p in cached),
          "unchanged files keep their cache entry")

    # Add a note and confirm it lands.
    path = os.path.join(news.root, "Later.md")
    with open(path, "w", encoding="utf-8") as f:
        f.write("A new note linking [[MOC]].\n")
    third = news.snapshot(now=NOW)
    ...
    os.remove(path)
    fourth = news.snapshot(now=NOW)
    eq(fourth["stats"], first["stats"], "deleting it puts everything back")
    check("Later.md" not in [os.path.basename(p) for p in news.cache],
...
def test_empty_news():
    ...
    It will be REJECTED by news_parse() as having no content — which is correct,
    ...
    try:
        snap = News(root).snapshot(now=NOW)
        eq(snap["stats"]["notes"], 0, "empty news has no notes")
        eq(snap["stats"]["links"], 0, "empty news has no links")
        eq(snap["graph"]["nodes"], [], "empty news has no graph")
        eq(snap["inbox_total"], 0, "empty news has no inbox")
...
def test_capture(tmpdir):
    root = os.path.join(tmpdir, "capture-news")
    os.makedirs(root)
    now = datetime.datetime(2026, 8, 10, 21, 4)
    ...
    check(os.path.realpath(escape).startswith(os.path.realpath(root)),
          f"capture cannot escape the news (wrote {escape})")
...
def test_payload_matches_the_wire_contract(news):
    """Every key the device's parser reads must be present and the right type."""
    snap = news.snapshot(now=NOW, agents=[])
    for key in ("schema", "news", "generated_at", "stats", "tags", "agents",
...
def main():
    ...
    try:
        ...
        os.makedirs(root)
        build_news(root)
        news = News(root, inbox_tag="#todo")

        test_strip_code()
        test_parse_note()
        test_snapshot(news)
        test_graph(news)
        test_graph_is_deterministic(news)
        test_inbox(news)
        test_titles_disambiguate_on_collision()
        test_graph_titles_disambiguate_only_against_the_drawn_nodes()
        test_recent(news)
        test_incremental_rescan(news)
        test_empty_news()
        test_agents(tmpdir)
        test_slugify()
        test_capture(tmpdir)
        test_agent_status_cli(tmpdir)
        test_glyph_check(news)
        test_payload_matches_the_wire_contract(news)
    finally:
        shutil.rmtree(tmpdir)

    print(f"\nnews_server: {CHECKS[0]} checks, {len(FAILURES)} failures")
