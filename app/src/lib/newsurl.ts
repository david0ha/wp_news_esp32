// Client-side validation of the news snapshot URL, mirroring the firmware's
// `prov_validate_news_url()` (components/provisioning/prov_config.c).
//
...
// write comes back as the single code `news_url_invalid`, which cannot say *what* was wrong.
//
...
export interface NewsUrlResult {
  ok: boolean
 ...
 * looks in the field.
 */
export function validateNewsUrl(input: string): NewsUrlResult {
  const url = (input ?? '').trim()
  if (url.length === 0) return { ok: true, value: '', empty: true }

  if (utf8Length(url) > NEWS_URL_MAX_LEN) return { ok: false, error: 'too_long' }
  ...
  // "http://" and "http:///news.json" both parse as URLs and both fail at connect time with an
