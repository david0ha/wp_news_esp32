...
describe('normalizeBaseUrl', () => {
  it('defaults the scheme to http:// for a bare host', () => {
    expect(normalizeBaseUrl('claudepost.local')).toEqual({ ok: true, value: 'http://claudepost.local' })
  })
  ...
  it('lower-cases the host and strips a trailing slash + path', () => {
    expect(normalizeBaseUrl('http://ClaudePost.local/api/')).toEqual({
      ok: true,
      value: 'http://claudepost.local',
    })
  })
...
describe('discoverDevice', () => {
  ...
  it('resolves the first candidate when the saved IP is reachable', async () => {
    ...
    const found = await discoverDevice(['192.168.0.42', 'http://claudepost.local'], { fetchImpl })
  ...
  it('falls through to the second candidate when the first fails', async () => {
    const { fetchImpl } = fakeFetch({
      ...
      'http://claudepost.local/api/info': true,
    })
    const found = await discoverDevice(['192.168.0.42', 'http://claudepost.local'], { fetchImpl })
    expect(found).toBe('http://claudepost.local')
  })
  ...
  it('returns null when every candidate fails', async () => {
    const { fetchImpl } = fakeFetch({
      ...
      'http://claudepost.local/api/info': false,
    })
    const found = await discoverDevice(['192.168.0.42', 'http://claudepost.local'], { fetchImpl })
  ...
  it('de-dupes equivalent candidates so a host is probed once', async () => {
    const { fetchImpl, calls } = fakeFetch({ 'http://claudepost.local/api/info': true })
    const found = await discoverDevice(
      ['claudepost.local', 'http://claudepost.local/'],
      { fetchImpl },
    )
    expect(found).toBe('http://claudepost.local')
