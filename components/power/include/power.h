 ...
 * gone, the 960,000-byte framebuffer is gone. `sizeof(news_t)` is 32,932 bytes
...
bool     power_deep_sleep_enabled(void);      /* CONFIG_WP_NEWS_DEEP_SLEEP */
uint32_t power_default_sleep_seconds(void);   /* CONFIG_WP_NEWS_SLEEP_SECONDS */
uint32_t power_awake_window_seconds(void);    /* CONFIG_WP_NEWS_AWAKE_WINDOW_SECONDS */
