 ...
 *
 * This is a SUMMARY, not the news snapshot. The phone does not need the graph
...
typedef struct {
    ...
    /* --- the news snapshot on the glass --- */
    bool news_valid;
    bool demo;                          /* rendered from the built-in sample */
    char news[DEV_NEWS_MAXLEN];
