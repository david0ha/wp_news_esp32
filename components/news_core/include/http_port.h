 ...
 * has. Reading it as an outage would count a failure, lengthen the retry
 * backoff and eventually badge the sheet OFFLINE, every time the server
 * correctly said nothing had changed: a board that works worse the better the
 * server behaves, and nothing in the log to say so. http_get() and
