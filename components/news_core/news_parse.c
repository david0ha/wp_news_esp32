...
static void parse_stats(const cJSON *root, news_t *v)
{
    ...
    if (n > NEWS_DAILY_DAYS) n = NEWS_DAILY_DAYS;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (val < 0) val = 0;
        v->stats.daily[NEWS_DAILY_DAYS - n + i] = val;
    }
}

static void parse_tags(const cJSON *root, news_t *v)
{
    const cJSON *arr = jarr(root, "tags");
    if (!arr) return;
    const cJSON *e = NULL;
    cJSON_ArrayForEach(e, arr) {
        if (v->tag_count >= NEWS_TAGS_MAX) break;
        if (!cJSON_IsObject(e)) continue;
        const char *name = jstr(e, "name");
        if (!name[0]) continue;
        news_tag_t *t = &v->tags[v->tag_count++];
        news_str_copy(t->name, sizeof(t->name), name);
        t->count = juint(e, "count", 0);
    }
}

static void parse_agents(const cJSON *root, news_t *v)
{
    const cJSON *arr = jarr(root, "agents");
    if (!arr) return;
    const cJSON *e = NULL;
    cJSON_ArrayForEach(e, arr) {
        if (v->agent_count >= NEWS_AGENTS_MAX) break;
        if (!cJSON_IsObject(e)) continue;
        const char *name = jstr(e, "name");
        if (!name[0]) continue;

        news_agent_t *a = &v->agents[v->agent_count++];
        news_str_copy(a->name, sizeof(a->name), name);
        a->state = news_agent_state_from(jstr(e, "state"));
        news_str_copy(a->last_run, sizeof(a->last_run), jstr(e, "last_run"));
        a->processed = juint(e, "processed", 0);
        a->queued    = juint(e, "queued", 0);
        news_str_copy(a->note, sizeof(a->note), jstr(e, "note"));
 ...
 * at most NEWS_NODES_MAX, so the O(n^2) is 200 comparisons. */
static void sort_nodes(news_t *v, uint8_t *remap)
{
    for (int i = 0; i < v->node_count; i++) remap[i] = (uint8_t)i;

    for (int i = 1; i < v->node_count; i++) {
        news_node_t key = v->nodes[i];
        uint8_t      kid = remap[i];
        int j = i - 1;
...
static void parse_graph(const cJSON *root, news_t *v)
{
    ...
    int wire_id[NEWS_NODES_MAX];

    const cJSON *arr = jarr(g, "nodes");
    const cJSON *e = NULL;
    if (arr) {
        cJSON_ArrayForEach(e, arr) {
            if (v->node_count >= NEWS_NODES_MAX) break;
            if (!cJSON_IsObject(e)) continue;
            const char *title = jstr(e, "title");
            if (!title[0]) continue;
            wire_id[v->node_count] = jint(e, "id", v->node_count);
            news_node_t *n = &v->nodes[v->node_count++];
            news_str_copy(n->title, sizeof(n->title), title);
            n->deg = juint(e, "deg", 0);
        }
    }
    ...
    uint8_t remap[NEWS_NODES_MAX];              /* remap[new] = old */
    sort_nodes(v, remap);

    uint8_t post_of_pre[NEWS_NODES_MAX];        /* the inverse */
    for (int i = 0; i < pre_count; i++) post_of_pre[remap[i]] = (uint8_t)i;

    arr = jarr(g, "edges");
    if (!arr) return;
    cJSON_ArrayForEach(e, arr) {
        if (v->edge_count >= NEWS_EDGES_MAX) break;
...
static void parse_recent(const cJSON *root, news_t *v)
{
    const cJSON *arr = jarr(root, "recent");
    if (!arr) return;
    const cJSON *e = NULL;
    cJSON_ArrayForEach(e, arr) {
        if (v->recent_count >= NEWS_RECENT_MAX) break;
        if (!cJSON_IsObject(e)) continue;
        const char *title = jstr(e, "title");
        if (!title[0]) continue;
        news_note_t *n = &v->recent[v->recent_count++];
        news_str_copy(n->title, sizeof(n->title), title);
        news_str_copy(n->time, sizeof(n->time), jstr(e, "time"));
        n->links = juint(e, "links", 0);
    }
}

static void parse_inbox(const cJSON *root, news_t *v)
{
    ...
    const cJSON *e = NULL;
    cJSON_ArrayForEach(e, arr) {
        if (v->inbox_count >= NEWS_INBOX_MAX) break;
        if (!cJSON_IsObject(e)) continue;
        const char *title = jstr(e, "title");
        if (!title[0]) continue;
        news_inbox_t *n = &v->inbox[v->inbox_count++];
        news_str_copy(n->title, sizeof(n->title), title);
...
bool news_parse(const char *json, size_t len, news_t *out)
{
    ...
    news_str_copy(v.news, sizeof(v.news), jstr(root, "news"));
    ...
    /* A well-formed object that carries no news content at all is a rejection,
