...
static agent_row_t s_rows[NEWS_AGENTS_MAX];
 ...
 * news_agent_state_name() (for /api) and this (for the panel) are separate. */
...
lv_obj_t *ui_page_agents_create(lv_obj_t *par)
{
    ...
    for (int i = 0; i < NEWS_AGENTS_MAX; i++) {
         ...
         * a news with two agents does not get four empty rules. */
...
void ui_page_agents_update(const news_t *v)
{
    if (!s_root) return;

    int n = (v && v->valid) ? v->agent_count : 0;
    if (n > NEWS_AGENTS_MAX) n = NEWS_AGENTS_MAX;

    ui_show(s_empty, n == 0);
    if (n == 0) {
        ui_set(s_head, S_PAGE_AGENTS);
        ui_set(s_sync, "");
        for (int i = 0; i < NEWS_AGENTS_MAX; i++) show_row(&s_rows[i], false);
        return;
    }

    ui_setf(s_head, "%s · %d / %d %s",
            S_PAGE_AGENTS, news_running_agents(v), n, S_AGENTS_RUNNING);
