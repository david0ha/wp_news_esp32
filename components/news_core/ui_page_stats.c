...
#define ACT_SLOT_W      (CW / NEWS_DAILY_DAYS)        /* 88  */
...
static lv_obj_t *s_bar[NEWS_DAILY_DAYS];
static lv_obj_t *s_bar_val[NEWS_DAILY_DAYS];

static lv_obj_t *s_tag_name[NEWS_TAGS_MAX];
static lv_obj_t *s_tag_bar[NEWS_TAGS_MAX];
static lv_obj_t *s_tag_cnt[NEWS_TAGS_MAX];
/* Shown instead of six blank rows when the news has no tags at all — which a
 * real news very often does, and which otherwise leaves a heading standing
...
void ui_page_stats_update(const news_t *v)
{
    if (!s_root) return;
