 ...
 *
 * The news's highest-degree notes and the links between them. Where the
...
static lv_obj_t *s_label[NEWS_NODES_MAX];
 ...
 * already holds. */
static graph_pos_t  s_pos[NEWS_NODES_MAX];
static int          s_pos_n;
static news_edge_t s_edges[NEWS_EDGES_MAX];
static int          s_edge_n;
...
lv_obj_t *ui_page_graph_create(lv_obj_t *par)
{
    ...
    for (int i = 0; i < NEWS_NODES_MAX; i++) {
...
void ui_page_graph_update(const news_t *v)
{
    if (!s_root) return;
    ...
    if (!v || !v->valid || v->node_count == 0) {
        ...
        for (int i = 0; i < NEWS_NODES_MAX; i++) ui_show(s_label[i], false);
    ...
    s_pos_n = ui_graph_layout(v, GR_CANVAS_W, GR_CANVAS_H, s_pos, NEWS_NODES_MAX);

    s_edge_n = v->edge_count;
    if (s_edge_n > NEWS_EDGES_MAX) s_edge_n = NEWS_EDGES_MAX;
    memcpy(s_edges, v->edges, sizeof(news_edge_t) * (size_t)s_edge_n);
