/*
 * ui_news.c — the header, the footer, the overlay, and which page is on top.
 *
 ...
 * rectangle that does not move when the news name gets longer; and every slot
 ...
 *
 *   14  brand    WP NEWS               104     ->118   gap 4
 *   122 news    <name>, ellipsized     116     ->238   gap 6
...
/* The news name is the header's only variable-width text, so it gets its own
 * slot and its own ellipsis rather than being concatenated onto the brand — a
 * combined "WP NEWS · <name>" label ellipsizes the brand away first, which
 * reads as a device that has forgotten what it is. */
static void refresh_brand(void)
{
    ui_set(s_news, (s_have_data && s_data.news[0]) ? s_data.news : "");
}
...
static void build_header(lv_obj_t *par)
{
    ...
    s_news = ui_lab_w(par, H_NEWS_X, 14, H_NEWS_W, UI_F_BODY,
                       LV_TEXT_ALIGN_LEFT, "");
...
void ui_news_set_overlay(const char *title, const char *body)
{
    if (!s_overlay) return;
    if (!title && !body) {
        ui_show(s_overlay, false);
...
void ui_news_create(lv_obj_t *parent)
{
    ...
    /* Assume online until told otherwise: ui_news_create runs before the first
...
void ui_news_set_data(const news_t *v)
{
    ...
    const news_t *arg = s_have_data ? &s_data : NULL;
...
void ui_news_header_area(int *x1, int *y1, int *x2, int *y2)
{
    /* The whole strip, including the rule under it — see ui_news.h for why
