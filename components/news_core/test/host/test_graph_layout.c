 ...
 * hands them over. */
static void make(news_t *v, int n, int top_deg)
{
    memset(v, 0, sizeof(*v));
    v->valid = true;
    v->node_count = n;
...
static void test_everything_stays_on_the_canvas(void)
{
    ...
    for (size_t s = 0; s < sizeof(sizes) / sizeof(sizes[0]); s++) {
        for (int n = 1; n <= NEWS_NODES_MAX; n++) {
            news_t v;
            make(&v, n, 30);
            graph_pos_t pos[NEWS_NODES_MAX];
            int got = ui_graph_layout(&v, sizes[s][0], sizes[s][1], pos, NEWS_NODES_MAX);
            CHECK_INT(got, n);
            char label[64];
...
static void test_layout_is_deterministic(void)
{
    ...
    graph_pos_t a[NEWS_NODES_MAX], b[NEWS_NODES_MAX];
    int na = ui_graph_layout(&v, 620, 360, a, NEWS_NODES_MAX);
    int nb = ui_graph_layout(&v, 620, 360, b, NEWS_NODES_MAX);
    CHECK_INT(na, nb);
...
static void test_biggest_hub_is_the_centre(void)
{
    news_t v;
    make(&v, 10, 24);
    graph_pos_t p[NEWS_NODES_MAX];
    int n = ui_graph_layout(&v, 620, 360, p, NEWS_NODES_MAX);
    CHECK_INT(n, 10);
...
{
    /* A news where every hub has the same degree is not exotic — it is what a
     * small, tidy news looks like. The radius scale divides by (hi - lo). */
    news_t v;
    memset(&v, 0, sizeof(v));
    v.valid = true;
    v.node_count = 6;
...
static void test_degenerate_inputs(void)
{
    ...
    CHECK_INT(ui_graph_layout(NULL, 620, 360, p, NEWS_NODES_MAX), 0);
    CHECK_INT(ui_graph_layout(&v, 620, 360, NULL, NEWS_NODES_MAX), 0);
    CHECK_INT(ui_graph_layout(&v, 0, 360, p, NEWS_NODES_MAX), 0);
    CHECK_INT(ui_graph_layout(&v, 620, 0, p, NEWS_NODES_MAX), 0);
    CHECK_INT(ui_graph_layout(&v, 620, 360, p, 0), 0);

    /* An empty graph is a normal state (a brand-new news), not an error. */
    news_t empty;
    memset(&empty, 0, sizeof(empty));
    empty.valid = true;
    CHECK_INT(ui_graph_layout(&empty, 620, 360, p, NEWS_NODES_MAX), 0);
