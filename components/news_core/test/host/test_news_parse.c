...
static void test_fixture(void)
{
    ...
    for (int i = 0; i < NEWS_DAILY_DAYS; i++) CHECK_INT(v.stats.daily[i], daily[i]);
    ...
    CHECK_INT(news_running_agents(&v), 2);
    CHECK_INT(news_link_density_x100(&v), 274);
    CHECK_INT(news_orphan_rate_x10(&v), 26);
    CHECK_INT(news_daily_peak(&v), 12);
...
static void check_rejects_and_preserves(const char *label, const char *json, size_t len)
{
    ...
    news_t v;
    CHECK(news_parse(good, flen, &v) == true);
    uint32_t before = news_hash(&v);

    bool ok = news_parse(json, len, &v);
    if (ok) {
        g_fail++; g_total++;
...
static void test_oversized_arrays_are_capped(void)
{
    ...
    news_t v;
    CHECK(PARSE(json, &v) == true);
    CHECK_INT(v.tag_count, NEWS_TAGS_MAX);
    CHECK_INT(v.agent_count, NEWS_AGENTS_MAX);
    CHECK_INT(v.recent_count, NEWS_RECENT_MAX);
    CHECK_INT(v.inbox_count, NEWS_INBOX_MAX);
    CHECK_INT(v.node_count, NEWS_NODES_MAX);
    CHECK(v.edge_count <= NEWS_EDGES_MAX);
...
{
    /* NEWS_TITLE_MAX is a byte count and Hangul is three bytes a syllable, so
...
static void test_hash_is_content_addressed(void)
{
    ...
    CHECK(news_parse(json, len, &a) == true);
    CHECK(news_parse(json, len, &b) == true);
    CHECK_INT(news_hash(&a), news_hash(&b));

    /* And any visible change must move it. */
    b.stats.notes++;
    CHECK(news_hash(&a) != news_hash(&b));

    news_t c = a;
    c.agents[0].queued++;
    CHECK(news_hash(&a) != news_hash(&c));

    news_t d = a;
    d.recent[7].links++;
    CHECK(news_hash(&a) != news_hash(&d));

    news_t e = a;
    e.nodes[13].deg++;
    CHECK(news_hash(&a) != news_hash(&e));
...
static void test_hash_separates_adjacent_strings(void)
{
    ...
    news_str_copy(a.recent[0].title, NEWS_TITLE_MAX, "ab");
    news_str_copy(a.recent[1].title, NEWS_TITLE_MAX, "c");
    news_str_copy(b.recent[0].title, NEWS_TITLE_MAX, "a");
    news_str_copy(b.recent[1].title, NEWS_TITLE_MAX, "bc");
    CHECK(news_hash(&a) != news_hash(&b));
}
...
static void test_str_copy(void)
{
    ...
    char tiny[2];
    CHECK_INT(news_str_copy(tiny, sizeof(tiny), "가"), 0);
    CHECK_STR(tiny, "");
