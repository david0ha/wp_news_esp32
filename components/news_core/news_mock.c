 ...
 *
 * The board is a finished object with no PC running: when no news_url has been
 * provisioned, this is what is on the glass, with a DEMO badge in the header so
 * nobody mistakes it for their own news.
 *
 ...
 * test_news_mock.c parses that server's committed output and asserts the two
...
static void add_tag(news_t *v, const char *name, int count)
{
    if (v->tag_count >= NEWS_TAGS_MAX) return;
    news_tag_t *t = &v->tags[v->tag_count++];
    CP(t->name, name);
    t->count = count;
}

static void add_agent(news_t *v, const char *name, agent_state_t state,
                      const char *last_run, int processed, int queued,
                      int progress, const char *note)
{
    if (v->agent_count >= NEWS_AGENTS_MAX) return;
...
static void add_node(news_t *v, const char *title, int deg)
{
    if (v->node_count >= NEWS_NODES_MAX) return;
    news_node_t *n = &v->nodes[v->node_count++];
    CP(n->title, title);
    n->deg = deg;
}

static void add_edge(news_t *v, int a, int b)
{
    if (v->edge_count >= NEWS_EDGES_MAX) return;
...
static void add_recent(news_t *v, const char *time, const char *title, int links)
{
    if (v->recent_count >= NEWS_RECENT_MAX) return;
...
static void add_inbox(news_t *v, const char *title, int age_days)
{
    if (v->inbox_count >= NEWS_INBOX_MAX) return;
...
void news_mock(news_t *v)
{
    ...
    static const int daily[NEWS_DAILY_DAYS] = { 3, 9, 12, 4, 0, 7, 6 };
