...
static lv_obj_t *s_rc_time[NEWS_RECENT_MAX];
static lv_obj_t *s_rc_title[NEWS_RECENT_MAX];
static lv_obj_t *s_rc_links[NEWS_RECENT_MAX];
static lv_obj_t *s_rc_empty;

static lv_obj_t *s_ib_box[NEWS_INBOX_MAX];
static lv_obj_t *s_ib_title[NEWS_INBOX_MAX];
static lv_obj_t *s_ib_age[NEWS_INBOX_MAX];
...
lv_obj_t *ui_page_notes_create(lv_obj_t *par)
{
    ...
    for (int i = 0; i < NEWS_RECENT_MAX; i++) {
    ...
    for (int i = 0; i < NEWS_INBOX_MAX; i++) {
        int y = NT_ROW_Y + i * NT_ROW_H;
...
void ui_page_notes_update(const news_t *v)
{
    if (!s_root) return;

    int nr = (v && v->valid) ? v->recent_count : 0;
    int ni = (v && v->valid) ? v->inbox_count : 0;
    if (nr > NEWS_RECENT_MAX) nr = NEWS_RECENT_MAX;
    if (ni > NEWS_INBOX_MAX)  ni = NEWS_INBOX_MAX;
    ...
    for (int i = 0; i < NEWS_RECENT_MAX; i++) {
        ...
        if (!on) continue;
        const news_note_t *n = &v->recent[i];
    ...
    for (int i = 0; i < NEWS_INBOX_MAX; i++) {
        ...
        if (!on) continue;
        const news_inbox_t *n = &v->inbox[i];
