 ...
 * one means a news of uniformly-linked notes still shows *some* variation
 ...
 * grow with circumference; with NEWS_NODES_MAX at 14 only the first two rings
 * are ever used, but the arithmetic does not depend on that. */
#define RING_MAX 4
static const int RING_CAP[RING_MAX] = { 6, 10, 14, 18 };

int ui_graph_layout(const news_t *v, int w, int h, graph_pos_t *out, int out_max)
{
