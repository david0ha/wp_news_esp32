...
`sizeof(news_t)` is 32,932 bytes and RTC slow memory is 8 KB, so the snapshot has
...
|---|---|---|
| `CONFIG_WP_NEWS_SLEEP_SECONDS` | Kconfig, default 900 | reflashing |
...
interval is the thing that will be resented within a month. `CONFIG_WP_NEWS_DEEP_SLEEP`
...
| `Kconfig.projbuild` | `WP_NEWS_DEEP_SLEEP` (off), `WP_NEWS_SLEEP_SECONDS` (900) |
