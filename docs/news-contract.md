...
| | |
|---|---|
| `tools/mock_news_server.py` | the reference producer, and a runnable server |
| `components/news_core/news_parse.c` | the consumer |
| `components/news_core/news_mock.c` | the built-in demo snapshot |

(`tools/news_server.py` is a fourth, but it produces whatever a real news happens to contain, so
it cannot be pinned against a fixture the way these three are. It has its own tests instead.)

`test_news_mock.c` parses the server's committed output
(`components/news_core/test/host/fixtures/news.json`) and asserts it fingerprints identically to
...
```
GET <news_url>          every CONFIG_WP_NEWS_POLL_SECONDS (default 300)
```
...
**Leaving it empty is a complete configuration.** The board then renders `news_mock.c` with a
...
| Field | Type | Notes |
|---|---|---|
| `news` | string | shown in the header, ellipsized to fit |
...
| `tags[].name` | string | up to `NEWS_TAGS_MAX` (6) shown |
...
**A payload with no news content at all is rejected**, and rejection means *the previous snapshot
...
python3 tools/mock_news_server.py --live          # numbers drift on every request
...
```bash
NEWS_URL=http://localhost:8123/news.json ./sim/sim.sh
```
...
`tools/news_server.py` walks an actual news on disk and serves this contract from it. It is
read-only — it opens `.md` files and writes nothing.

```bash
python3 tools/news_server.py ~/Documents/MyNews        # serve on :8123
python3 tools/news_server.py ~/Documents/MyNews --dump # print the payload
python3 tools/test_news_server.py                       # its tests
```

The definitions it uses are WP News's where WP News has one, and stated in the script's docstring
...
- **A colliding note name is prefixed with its folder** (`docs/README`), because a real news has
...
`~/Library/LaunchAgents/local.wpnews.news.plist`:

```xml
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
  <key>Label</key><string>local.wpnews.news</string>
  <key>ProgramArguments</key><array>
    <string>/usr/bin/python3</string>
    <string>/Users/YOU/Documents/wp_news_esp32/tools/news_server.py</string>
    <string>/Users/YOU/Documents/MyNews</string>
  </array>
  <key>RunAtLoad</key><true/>
  <key>KeepAlive</key><true/>
  <key>StandardErrorPath</key><string>/tmp/wpnews-news.log</string>
</dict></plist>
```

```bash
launchctl load ~/Library/LaunchAgents/local.wpnews.news.plist
```
...
A plugin inside WP News, a cron job over the news directory, and a shell script that greps
