...
`CONFIG_WP_NEWS_AWAKE_WINDOW_SECONDS` (120 by default), and every request restarts that window, so an
...
curl -X POST http://wpnews.local/api/sleep -d '{"seconds":1800}'
curl -s http://wpnews.local/api/state | jq .power

# back to the build-time default interval
curl -X POST http://wpnews.local/api/sleep -d '{"seconds":0}'
